...
import org.lightpaycashj.core.listeners.BlockChainListener;
import org.lightpaycashj.store.FlatDB;
import org.lightpaycashj.store.HashStore;
import org.lightpaycashj.store.MasternodeDB;
 ...
 * lightpaycashj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating lightpaycashj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use lightpaycashj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a lightpaycashj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using lightpaycashj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing lightpaycashj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.lightpaycashj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.lightpaycashj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.lightpaycashj.core.NetworkParameters} specified when this context was (auto) created. The
